...
CorrKoef = data.corr()
FielDrop = [i for i in CorrKoef if CorrKoef[i].isnull().drop_duplicates().values[0]] #пробегаем матрицу корреляций, если в ней встречается значение NaN - скорее всего, столбец содержит одно повторяющееся значения, складываем в FielDrop
CorrField = []
CorrDropFiels = []
...
FielDrop = FielDrop + CorrDropFiels#объединяем контейнеры CorrDropFiels and FielDrop
#print('+ те столбцы, что мы удалили в нашем цикле',FielDrop)
...
'''for el in ydata.flat:
    print(el)'''
'''model = ensemble.GradientBoostingClassifier(n_estimators=80,learning_rate=0.1, subsample=0.5,random_state=11)
